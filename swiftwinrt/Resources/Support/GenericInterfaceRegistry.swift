...
public enum GenericInterfaceRegistry {
    private static var factories: [SUPPORT_MODULE.IID: (AnyObject) -> IUnknownRef?] = [:]
    private static var lock = SRWLock()

    public struct Registrar: ~Copyable {
        fileprivate init() {}

        public func register(_ iid: SUPPORT_MODULE.IID, _ factory: @escaping (AnyObject) -> IUnknownRef?) {
    ...
    static func queryInterface(_ obj: AnyObject, _ iid: SUPPORT_MODULE.IID) -> IUnknownRef? {
