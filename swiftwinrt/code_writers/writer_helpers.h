...
%func removeLast() {
    ...
    template<typename T>
    static void write_abi_val_assignment(writer& w, T const& return_type, std::string_view return_param_name)
    {
    ...
    static void write_abi_ret_val(writer& w, function_def signature)
    {
        ...
        for (auto& param : signature.params)
        {
            ...
            if (param.out())
            {
                ...
                write_abi_val_assignment(w, param, return_param_name);
            }
        ...
        if (signature.return_type)
        {
            ...
            auto return_param_name = "$" + std::to_string(param_number);
            write_abi_val_assignment(w, signature.return_type.value(), return_param_name);
        }
    }
    ...
    template <typename T>
    static void write_vtable_body(writer& w, T const& type, std::vector<named_interface_info> interfaces)
    {
        constexpr bool isInterface = std::is_same_v<T, interface_type>;
        constexpr bool isDelegate = std::is_same_v<T, delegate_type>;
        constexpr bool isGeneric = std::is_same_v<T, generic_inst>;
